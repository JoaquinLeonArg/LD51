...
class ResourceData:
	...
	func gain_resource(_resource_type: int, _amount: int):
		...
		self.resources[_resource_type] = max(self.resources[_resource_type], self.resource_max[_resource_type])
		emit_signal("resource_changed", _resource_type, old, self.resources[_resource_type], resource_production[_resource_type] - resource_maintenance[_resource_type])
	func resource_tick():
		for resource in [ResourceType.PEOPLE, ResourceType.FOOD]:
			...
			self.resources[resource] = min(self.resources[resource], 0)
			emit_signal("resource_changed", resource, old, self.resources[resource], resource_production[resource] - resource_maintenance[resource])
